...
export default function DocumentManager({ employee, onClose, isEmbedded = false }) {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await api.fetchJson(`/api/documents/${id}`, { method: 'DELETE' });
            loadDocs();
        } catch (e) {
            alert(e.message);
        }
    ...
    const content = (
        <div className={isEmbedded ? "" : "modal"} style={isEmbedded ? {} : { maxWidth: '800px', width: '90%' }}>
            <div className="flex-row flex-between" style={{ borderBottom: '1px solid var(--border)', paddingBottom: '10px' }}>
                <h3>Documents for {employee.name}</h3>
                {!isEmbedded && <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer', color: 'var(--text-light)' }}>×</button>}
            </div>

            <div className="grid-2" style={{ marginTop: '20px', gap: '30px' }}>
                {/* Left side: Upload Form */}
                <div style={{ borderRight: isEmbedded ? 'none' : '1px solid var(--border)', paddingRight: '20px' }}>
